 ...
 * Open investigations.
 *
 * Field names follow the FastAPI contract in `lib/api/schema.js` —
 * `case_id`, `description`, `status`, `notes` — rather than names an
 * isolated frontend would have chosen. `status` uses the wire values
 * (`pending_review`), with display labels in `STATUS_LABELS`.
 */
